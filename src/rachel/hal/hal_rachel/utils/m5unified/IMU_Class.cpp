...
namespace m5
{
    ...
    //           _internal_axisorder_fixed[sensor_index_accel] = (internal_axisorder_t)(axis_invert_x | axis_invert_z); //
    //           X軸,Z軸反転 _internal_axisorder_fixed[sensor_index_gyro ] = (internal_axisorder_t)(axis_invert_x |
    //           axis_invert_z); // X軸,Z軸反転
    //         }
    //       }
    //     }
    ...
    //         _internal_axisorder_fixed[sensor_index_mag] = (internal_axisorder_t)(axis_invert_y | axis_invert_z); //
    //         Y軸,Z軸反転
    //       }
    //     }
    ...
    //           _internal_axisorder_fixed[sensor_index_mag] = (internal_axisorder_t)(axis_invert_x | axis_invert_z); //
    //           X軸,Z軸反転
    //         }
    //       }
    //     }
    ...
    //           _internal_axisorder_fixed[sensor_index_mag ] = (internal_axisorder_t)(axis_order_yxz | axis_invert_z); //
    //           Y軸X軸を入替, Z軸反転
    //         }
    //       }
    //     }
    ...
    bool IMU_Class::begin(I2C_Class* i2c)
    {
        ...
        if (_imu == imu_t::imu_none)
        {
            ...
            else
            {
                ...
                _imu = imu_t::imu_bmi270;
                // CoreS3 BMI270 + BMM150 構成では、地磁気のY軸Z軸をそれぞれ反転する
                _internal_axisorder_fixed[sensor_index_mag] =
                    (internal_axisorder_t)(axis_invert_y | axis_invert_z); // Y軸,Z軸反転
            }
        }
    ...
    void IMU_Class::_update_convert_param(void)
    {
        if (_imu_instance[0])
        {
            _imu_instance[0]->getConvertParam(&_convert_param);
        }
        if (_imu_instance[1])
        {
            _imu_instance[1]->getConvertParam(&_convert_param);
        }
    ...
    void IMU_Class::setCalibration(uint8_t accel, uint8_t gyro, uint8_t mag)
    {
        ...
        _calibration_flg =
            (sensor_mask_t)((accel ? sensor_mask_accel : 0) | (gyro ? sensor_mask_gyro : 0) | (mag ? sensor_mask_mag : 0));
    }
    ...
    { // データの取り出し順序を指定する
        ...
        {
        case 0 << 0 | 1 << 2 | 2 << 4:
            result = axis_order_xyz;
            break;
        case 0 << 0 | 2 << 2 | 1 << 4:
            result = axis_order_xzy;
            break;
        case 1 << 0 | 0 << 2 | 2 << 4:
            result = axis_order_yxz;
            break;
        case 1 << 0 | 2 << 2 | 0 << 4:
            result = axis_order_yzx;
            break;
        case 2 << 0 | 0 << 2 | 1 << 4:
            result = axis_order_zxy;
            break;
        case 2 << 0 | 1 << 2 | 0 << 4:
            result = axis_order_zyx;
            break;
        default:
            return false;
        }
    ...
    { // システムの軸設定とユーザの軸設定を反映したデータの取り出し順序を求める
        ...
        for (std::size_t sensor = 0; sensor < 3; ++sensor)
        {
            ...
            { // ユーザが要求している軸順と反転指定を求める
                ...
                // invert_user, axis_fixed_index, invert_fixed, result);
            }
        }
    ...
    int32_t IMU_Class::getOffsetData(size_t index) { return index < 9 ? _offset_data.sensor[index / 3].value[index % 3] : 0; }

    int16_t IMU_Class::getRawData(size_t index) { return index < 9 ? _raw_data.value[index] : 0; }
