...
namespace m5
{
    ...
    bool I2C_Class::begin(void) { return m5gfx::i2c::init(_port_num).has_value(); }

    bool I2C_Class::release(void) const { return m5gfx::i2c::release(_port_num).has_value(); }
    ...
    bool I2C_Class::stop(void) const { return m5gfx::i2c::endTransaction(_port_num).has_value(); }

    bool I2C_Class::write(std::uint8_t data) const { return m5gfx::i2c::writeBytes(_port_num, &data, 1).has_value(); }
    ...
    bool I2C_Class::writeRegister(std::uint8_t address,
                                  std::uint8_t reg,
                                  const std::uint8_t* __restrict__ data,
                                  std::size_t length,
                                  std::uint32_t freq) const
    {
        return m5gfx::i2c::beginTransaction(_port_num, address, freq, false).has_value() &&
               ...
               m5gfx::i2c::writeBytes(_port_num, data, length).has_value() && m5gfx::i2c::endTransaction(_port_num).has_value();
    }

    bool I2C_Class::readRegister(
        std::uint8_t address, std::uint8_t reg, std::uint8_t* __restrict__ result, std::size_t length, std::uint32_t freq) const
    {
