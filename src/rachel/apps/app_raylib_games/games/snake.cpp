...
#define SQUARE_SIZE 12
    ...
    void InitGame(void)
    {
        ...
        for (int i = 0; i < SNAKE_LENGTH; i++)
        {
            ...
            if (i == 0)
                _data->snake[i].color = DARKBLUE;
            else
                _data->snake[i].color = BLUE;
        }
        ...
        if (!_data->gameOver)
        {
            ...
            if (!_data->pause)
            {
                ...
                for (int i = 0; i < _data->counterTail; i++)
                    _data->snakePosition[i] = _data->snake[i].position;
                ...
                if ((_data->framesCounter % 5) == 0)
                {
                    for (int i = 0; i < _data->counterTail; i++)
                    {
                        if (i == 0)
                        {
                            ...
                            _data->snake[i].position = _data->snakePosition[i - 1];
                    }
                }

                // Wall behaviour
                if (((_data->snake[0].position.x) > (screenWidth - _data->offset.x)) ||
                    ((_data->snake[0].position.y) > (screenHeight - _data->offset.y)) || (_data->snake[0].position.x < 0) ||
                    (_data->snake[0].position.y < 0))
                {
                    _data->gameOver = true;
                }
                ...
                {
                    if ((_data->snake[0].position.x == _data->snake[i].position.x) &&
                        (_data->snake[0].position.y == _data->snake[i].position.y))
                        _data->gameOver = true;
                }
                ...
                if (!_data->fruit.active)
                {
                    ...
                    _data->fruit.position =
                        (Vector2){GetRandomValue(0, (screenWidth / SQUARE_SIZE) - 1) * SQUARE_SIZE + _data->offset.x / 2,
                                  GetRandomValue(0, (screenHeight / SQUARE_SIZE) - 1) * SQUARE_SIZE + _data->offset.y / 2};

                    for (int i = 0; i < _data->counterTail; i++)
                    {
                        while ((_data->fruit.position.x == _data->snake[i].position.x) &&
                               (_data->fruit.position.y == _data->snake[i].position.y))
                        {
                            _data->fruit.position = (Vector2){
                                GetRandomValue(0, (screenWidth / SQUARE_SIZE) - 1) * SQUARE_SIZE + _data->offset.x / 2,
                                GetRandomValue(0, (screenHeight / SQUARE_SIZE) - 1) * SQUARE_SIZE + _data->offset.y / 2};
                            i = 0;
                        }
                    }
                ...
                // Collision
                if ((_data->snake[0].position.x < (_data->fruit.position.x + _data->fruit.size.x) &&
                     (_data->snake[0].position.x + _data->snake[0].size.x) > _data->fruit.position.x) &&
                    (_data->snake[0].position.y < (_data->fruit.position.y + _data->fruit.size.y) &&
                     (_data->snake[0].position.y + _data->snake[0].size.y) > _data->fruit.position.y))
                {
    ...
    void DrawGame(void)
    {
        ...
        if (!_data->gameOver)
        {
            ...
            {
                DrawLineV((Vector2){SQUARE_SIZE * i + _data->offset.x / 2, _data->offset.y / 2},
                          (Vector2){SQUARE_SIZE * i + _data->offset.x / 2, screenHeight - _data->offset.y / 2},
                          LIGHTGRAY);
            }

            for (int i = 0; i < screenHeight / SQUARE_SIZE + 1; i++)
            {
                DrawLineV((Vector2){_data->offset.x / 2, SQUARE_SIZE * i + _data->offset.y / 2},
                          (Vector2){screenWidth - _data->offset.x / 2, SQUARE_SIZE * i + _data->offset.y / 2},
                          LIGHTGRAY);
            }

            // Draw snake
            for (int i = 0; i < _data->counterTail; i++)
                DrawRectangleV(_data->snake[i].position, _data->snake[i].size, _data->snake[i].color);
